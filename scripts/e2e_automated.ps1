...
Write-Host 'Running canonical E2E script...'
$token = Try-Login
...
Write-Host 'Canonical E2E finished.'
# Overwritten with clean, tested script content (canonical)
param(
    [int]$AmountCents = 4400,
    [switch]$Interactive,
    [switch]$SimulateWebhook = $false,
...
Write-Host 'Canonical E2E finished.'
# E2E automated script for donations
# Features:
# - Login as admin, create a Stripe Checkout session
# - Optionally open the Checkout URL interactively
# - Optionally simulate a signed Stripe webhook (requires STRIPE_WEBHOOK_SECRET)
# - Verify session via backend endpoint
# - Optional DB check via docker exec (if docker + container available)
...
function Check-Db-For-Session($sessionId) {
    ...
    $cmd = "docker exec -i $container psql -U postgres -d recetas -c \"$query\""
...
Write-Host "E2E Automated: amount(cents)=$AmountCents, Interactive=$Interactive, SimulateWebhook=$SimulateWebhook, ExportCsv=$ExportCsv, CheckDb=$CheckDb, ForceLocal=$ForceLocal"
...
Write-Host 'E2E automated script finished.'

param(
    [int]$AmountCents = 4400,
    [switch]$Interactive,        # si se quiere abrir la URL en navegador
...
Write-Host "E2E Automated: amount(cents)=$AmountCents, Interactive=$Interactive, SimulateWebhook=$SimulateWebhook"

$base='http://localhost:8081'
$envFile = Join-Path (Split-Path $PSScriptRoot) '..\.env' | Resolve-Path -ErrorAction SilentlyContinue
$envVars = @{}
param(
    [int]$AmountCents = 4400,
    [switch]$Interactive,
...
$envFile = Join-Path (Split-Path $PSScriptRoot) '..\.env' | Resolve-Path -ErrorAction SilentlyContinue
$envVars = @{}
if ($envFile) { $envVars = Read-EnvFile($envFile) }
...
function Compute-StripeSignature($secret, $payload) {
    ...
    $signed = "$ts.$payload"
    $h = New-Object System.Security.Cryptography.HMACSHA256([Text.Encoding]::UTF8.GetBytes($secret))
    $sigBytes = $h.ComputeHash([Text.Encoding]::UTF8.GetBytes($signed))
...
try {
    ...
    $create = Create-Session $token $AmountCents
    Write-Host "Create response:`n"
    Write-Host (ConvertTo-Json $create -Depth 6)

    $sessionId = $null
    if ($create.sessionId) { $sessionId = $create.sessionId }
    elseif ($create.sesion_pago -and $create.sesion_pago.sessionId) { $sessionId = $create.sesion_pago.sessionId }
    ...
    if ($SimulateWebhook) {
        ...
        if (-not $secret) {
            Write-Host 'STRIPE_WEBHOOK_SECRET not found in env or .env; cannot sign webhook. Aborting simulation.' -ForegroundColor Yellow
        } else {
    ...
    Write-Host (ConvertTo-Json $verify -Depth 8)
